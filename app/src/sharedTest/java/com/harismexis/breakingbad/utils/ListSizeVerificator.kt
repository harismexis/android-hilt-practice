...
import com.harismexis.breakingbad.parser.MockHerosParser
...
fun <T> verifyListSizeWhenAllIdsValid(items: List<T>) {
    verifyListSize(MockHerosParser.EXPECTED_NUM_ACTORS_WHEN_ALL_IDS_VALID, items)
}

fun <T> verifyListSizeWhenSomeIdsAbsent(items: List<T>) {
    verifyListSize(MockHerosParser.EXPECTED_NUM_ACTORS_WHEN_SOME_IDS_INVALID, items)
}

fun <T> verifyListSizeWhenSomeItemsEmpty(items: List<T>) {
    verifyListSize(MockHerosParser.EXPECTED_NUM_ACTORS_WHEN_SOME_EMPTY, items)
}

fun <T> verifyListSizeForNoData(items: List<T>) {
    verifyListSize(MockHerosParser.EXPECTED_NUM_ACTORS_WHEN_NO_DATA, items)
}